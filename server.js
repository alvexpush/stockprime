...
const verificationCodeHash=value=>crypto.createHmac("sha256",process.env.OTP_SECRET||(productionRuntime?"missing-production-otp-secret":"vanguardprime-local-development")).update(value).digest("hex");
...
function sessionCookie(token) {
  ...
  return `vanguardprime_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${sessionDays*86400}${secure}`;
}
function clearCookie() { return "vanguardprime_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0"; }
function adminCookie(token) { return `vanguardprime_admin_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800`; }
function clearAdminCookie() { return "vanguardprime_admin_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"; }
...
function pageRequest(url, defaultLimit=50, maxLimit=100) {
  ...
  return {limit,offset,queryLimit:limit+1};
}
function pageResult(rows,page) {
  return {items:rows.slice(0,page.limit),pagination:{limit:page.limit,offset:page.offset,hasMore:rows.length>page.limit}};
}
...
function currentUser(req) {
  const token = cookies(req).vanguardprime_session;
...
function requireAdmin(req,res){
  const token=cookies(req).vanguardprime_admin_session,session=token&&adminSessions.get(sha256(token));
...
async function sendPasswordReset(email,link){
  const sent=await sendTransactionalEmail({to:email,kind:"login_code_reset",subject:"Reset your Vanguard Prime login code",html:emailTemplate({eyebrow:"Security",title:"Reset your login code",intro:"We received a request to replace the private six-digit login code for your account.",actionUrl:link,actionLabel:"Choose a new login code",note:"This link expires in 30 minutes and works only once. If you did not request this, you can safely ignore this email."})});
...
async function sendWalletEventEmail(user,{type,status,amountCents,feeCents=0,currency="USD",method="",transactionId=""}){
  ...
  const intro=isWithdrawal&&status==="pending"?"A withdrawal has just been requested from your Vanguard Prime account. If this was not you, contact support immediately.":`Your ${label.toLowerCase()} status has been updated.`;
  return safeTransactionalEmail({to:user.email,kind:`${type}_${status}`,subject:`Vanguard Prime ${label.toLowerCase()} ${status}`,html:emailTemplate({eyebrow:"Wallet activity",title,intro,content,note:"For your security, Vanguard Prime will never ask for your login code by email."})});
}
async function marketNews(){
  ...
  if(process.env.ALPHA_VANTAGE_API_KEY){
    ...
    const response=await fetch(url,{headers:{"User-Agent":"Vanguard Prime/1.0"},signal:AbortSignal.timeout(10000)});
  ...
  if(process.env.FINNHUB_API_KEY){
    const response=await fetch(`https://finnhub.io/api/v1/news?category=general&token=${encodeURIComponent(process.env.FINNHUB_API_KEY)}`,{headers:{"User-Agent":"Vanguard Prime/1.0"},signal:AbortSignal.timeout(10000)});
    if(!response.ok)throw Object.assign(new Error(`News provider returned ${response.status}.`),{status:502});
...
async function api(req, res, url) {
  ...
  if(req.method==="GET"&&url.pathname==="/api/btc-price"){
    ...
    try{
      const response=await fetch("https://api.coinbase.com/v2/prices/BTC-USD/spot",{headers:{"Accept":"application/json","User-Agent":"Vanguard Prime/1.0"},signal:AbortSignal.timeout(8000)});
  ...
  if(req.method==="GET"&&url.pathname==="/api/crypto-prices"){
    ...
    try{const ids={BTC:"bitcoin",ETH:"ethereum",SOL:"solana",DOGE:"dogecoin",XRP:"ripple",ADA:"cardano",POL:"polygon-ecosystem-token",LTC:"litecoin"},response=await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${Object.values(ids).join(",")}&vs_currencies=usd`,{headers:{"Accept":"application/json","User-Agent":"Vanguard Prime/1.0"},signal:AbortSignal.timeout(8000)});if(!response.ok)throw new Error(`CoinGecko returned ${response.status}.`);const payload=await response.json();fresh=symbols.map(symbol=>({symbol,price:Number(payload?.[ids[symbol]]?.usd)})).filter(item=>Number.isFinite(item.price)&&item.price>0)}catch{}
    if(!fresh.length){source="Coinbase";const results=await Promise.allSettled(symbols.map(async symbol=>{const response=await fetch(`https://api.coinbase.com/v2/prices/${symbol}-USD/spot`,{headers:{"Accept":"application/json","User-Agent":"Vanguard Prime/1.0"},signal:AbortSignal.timeout(8000)});if(!response.ok)throw new Error(`${symbol} price provider returned ${response.status}.`);const payload=await response.json(),price=Number(payload?.data?.amount);if(!Number.isFinite(price)||price<=0)throw new Error(`${symbol} price is unavailable.`);return {symbol,price}}));fresh=results.filter(result=>result.status==="fulfilled").map(result=>result.value)}
  ...
  if(req.method==="POST"&&url.pathname==="/api/password/reset"){
    ...
    const updatedUser=db.prepare("SELECT email,name FROM users WHERE id=?").get(record.user_id);await safeTransactionalEmail({to:updatedUser.email,kind:"login_code_changed",subject:"Your Vanguard Prime login code was changed",html:emailTemplate({eyebrow:"Security alert",title:"Login code changed",intro:"Your private six-digit Vanguard Prime login code was changed successfully.",note:"All existing sessions were signed out. If you did not make this change, contact support immediately."})});
  ...
  if(req.method==="POST"&&url.pathname==="/api/password/change"){
    ...
    db.prepare("UPDATE users SET login_code_hash=?,updated_at=? WHERE id=?").run(await passwordHash(loginCode),now(),user.id);db.prepare("DELETE FROM sessions WHERE user_id=? AND token_hash<>?").run(user.id,sha256(cookies(req).vanguardprime_session));const emailSent=await safeTransactionalEmail({to:user.email,kind:"login_code_changed",subject:"Your Vanguard Prime login code was changed",html:emailTemplate({eyebrow:"Security alert",title:"Login code changed",intro:"Your private six-digit Vanguard Prime login code was changed successfully.",note:"Other signed-in sessions were closed. If you did not make this change, contact support immediately."})});return json(res,200,{message:"Login code changed successfully.",emailSent});
  }
  if(req.method==="POST"&&url.pathname==="/api/admin/login"){
    const input=await body(req),email=String(input.email||"").trim().toLowerCase(),password=String(input.password||""),expectedEmail=String(process.env.ADMIN_EMAIL||"admin@vanguardprime.test").toLowerCase(),expectedPassword=process.env.ADMIN_PASSWORD||"Admin123!";
  ...
  if(req.method==="POST"&&url.pathname==="/api/admin/logout"){const token=cookies(req).vanguardprime_admin_session;if(token)adminSessions.delete(sha256(token));return json(res,200,{message:"Signed out."},{"Set-Cookie":clearAdminCookie()})}
  ...
  if(req.method==="GET"&&url.pathname==="/api/admin/referrals"){
    if(!requireAdmin(req,res))return;const page=pageRequest(url),result=pageResult(db.prepare(`SELECT r.public_id AS id,r.referral_code,r.status,r.investment_status,r.qualifying_investment_cents,r.commission_cents,r.created_at,r.qualified_at,referrer.name AS referrer_name,referrer.email AS referrer_email,referred.name AS referred_name,referred.email AS referred_email FROM referrals r JOIN users referrer ON referrer.id=r.referrer_user_id JOIN users referred ON referred.id=r.referred_user_id ORDER BY r.created_at DESC LIMIT ? OFFSET ?`).all(page.queryLimit,page.offset),page);return json(res,200,{referrals:result.items,pagination:result.pagination});
  }
  if(req.method==="GET"&&url.pathname==="/api/admin/affiliate-withdrawals"){
    if(!requireAdmin(req,res))return;const page=pageRequest(url),result=pageResult(db.prepare("SELECT w.public_id AS id,w.amount_cents,w.method,w.destination,w.status,w.admin_note,w.created_at,w.updated_at,u.name,u.email FROM affiliate_withdrawals w JOIN users u ON u.id=w.user_id ORDER BY w.created_at DESC LIMIT ? OFFSET ?").all(page.queryLimit,page.offset),page);return json(res,200,{withdrawals:result.items,pagination:result.pagination});
  }
  ...
  if(req.method==="GET"&&url.pathname==="/api/admin/users"){
    if(!requireAdmin(req,res))return;const page=pageRequest(url),result=pageResult(db.prepare("SELECT u.public_id AS id,u.name,u.email,u.status,u.created_at AS joined,u.email_verified_at,k.status AS kyc_status FROM users u LEFT JOIN kyc_submissions k ON k.user_id=u.id ORDER BY u.created_at DESC LIMIT ? OFFSET ?").all(page.queryLimit,page.offset),page),users=result.items.map(user=>({...user,status:user.status[0].toUpperCase()+user.status.slice(1),emailVerified:Boolean(user.email_verified_at),kyc:user.kyc_status||"not_submitted",email_verified_at:undefined,kyc_status:undefined}));return json(res,200,{users,pagination:result.pagination});
  }
  if(req.method==="GET"&&url.pathname==="/api/admin/kyc"){
    if(!requireAdmin(req,res))return;const page=pageRequest(url),result=pageResult(db.prepare("SELECT k.*,u.public_id AS user_public_id,u.name AS user_name,u.email AS user_email FROM kyc_submissions k JOIN users u ON u.id=k.user_id ORDER BY CASE k.status WHEN 'pending' THEN 0 ELSE 1 END,k.submitted_at DESC LIMIT ? OFFSET ?").all(page.queryLimit,page.offset),page),submissions=result.items.map(row=>({...kycResponse(row),userId:row.user_public_id,userName:row.user_name,userEmail:row.user_email}));return json(res,200,{submissions,pagination:result.pagination});
  }
  if(req.method==="POST"&&/^\/api\/admin\/kyc\/[^/]+\/status$/.test(url.pathname)){
    const admin=requireAdmin(req,res);if(!admin)return;const id=decodeURIComponent(url.pathname.split("/")[4]),input=await body(req),status=String(input.status||"").toLowerCase(),reviewNote=String(input.reviewNote||"").trim().slice(0,1000);if(!["approved","rejected","resubmission_required"].includes(status))return json(res,422,{error:"Choose a valid KYC review status."});const submission=db.prepare("SELECT k.*,u.email,u.name,u.public_id AS user_public_id FROM kyc_submissions k JOIN users u ON u.id=k.user_id WHERE k.public_id=?").get(id);if(!submission)return json(res,404,{error:"KYC submission was not found."});const timestamp=now();db.prepare("UPDATE kyc_submissions SET status=?,review_note=?,reviewed_at=?,updated_at=? WHERE public_id=?").run(status,reviewNote||null,timestamp,timestamp,id);db.prepare("INSERT INTO audit_logs (actor_type,actor_id,action,entity_type,entity_id,details_json,ip_address,created_at) VALUES ('admin',?,'reviewed_kyc','kyc_submission',?,?,?,?)").run(admin.email,id,JSON.stringify({status,reviewNote}),requestIp(req),timestamp);const label=status==="approved"?"approved":status==="rejected"?"not approved":"returned for resubmission",emailSent=await safeTransactionalEmail({to:submission.email,kind:`kyc_${status}`,subject:`Your Vanguard Prime KYC application was ${label}`,html:emailTemplate({eyebrow:"Identity verification",title:`KYC application ${label}`,intro:status==="approved"?"Your identity verification has been approved.":"Your identity verification status has changed. Open your dashboard to review the details.",actionUrl:publicUrl(req,"/kyc.html"),actionLabel:"View KYC status",note:reviewNote||"Contact Vanguard Prime support if you need help."})});return json(res,200,{message:`KYC application ${label}.`,status,emailSent});
  }
  ...
  if(req.method==="GET"&&url.pathname==="/api/admin/investment-plans"){
    ...
    const plans=db.prepare("SELECT public_id,name,category,nav_cents,minimum_cents,maximum_cents,daily_return_bps,duration_days,projected_return_bps,management_fee_bps,risk_level,status,description FROM investment_plans ORDER BY status DESC,id").all();
    return json(res,200,{plans});
  }
  if(req.method==="POST"&&url.pathname==="/api/admin/investment-plans"){
    const admin=requireAdmin(req,res);if(!admin)return;const input=await body(req),id=String(input.id||""),name=String(input.name||"").trim(),category=String(input.category||"").trim(),nav=cents(input.nav),minimum=cents(input.minInvestment),maximum=cents(input.maxInvestment),dailyReturnBps=Math.round(Number(input.dailyReturn)*100),durationDays=Math.round(Number(input.durationDays)),returnBps=Math.round(Number(input.returnRate)*100),feeBps=Math.round(Number(input.managementFee||0)*100),risk=String(input.risk||"Medium"),status=String(input.status||"Active").toLowerCase()==="active"?"active":"inactive",description=String(input.description||"").trim();
    if(name.length<3)return json(res,422,{error:"Enter a plan name."});if(!category)return json(res,422,{error:"Enter a plan category."});if(!Number.isSafeInteger(nav)||nav<1)return json(res,422,{error:"Enter a valid NAV."});if(!Number.isSafeInteger(minimum)||minimum<100)return json(res,422,{error:"Enter a valid minimum investment."});if(!Number.isSafeInteger(maximum)||maximum<minimum)return json(res,422,{error:"Maximum investment must be equal to or greater than the minimum."});if(!Number.isSafeInteger(dailyReturnBps)||dailyReturnBps<0)return json(res,422,{error:"Enter a valid daily return."});if(!Number.isSafeInteger(durationDays)||durationDays<1||durationDays>3650)return json(res,422,{error:"Duration must be between 1 and 3650 days."});if(!Number.isSafeInteger(returnBps)||returnBps<0)return json(res,422,{error:"Enter a valid total return."});if(!Number.isSafeInteger(feeBps)||feeBps<0)return json(res,422,{error:"Enter a valid management fee."});if(!["Low","Medium","High"].includes(risk))return json(res,422,{error:"Choose a valid risk level."});
    const timestamp=now(),planPublicId=id||publicId("PLAN");
    db.prepare(`INSERT INTO investment_plans (public_id,name,category,nav_cents,minimum_cents,maximum_cents,daily_return_bps,duration_days,projected_return_bps,management_fee_bps,risk_level,status,description,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  ...
  if(req.method==="GET"&&url.pathname==="/api/admin/payments"){
    ...
    const payments=db.prepare(`SELECT t.public_id AS id,t.amount_cents,t.fee_cents,t.currency,t.method,t.reference,t.status,t.created_at AS date,u.email AS customer,u.name AS customer_name
      ...
      ORDER BY t.created_at DESC LIMIT ? OFFSET ?`).all(page.queryLimit,page.offset),result=pageResult(payments,page);
    return json(res,200,{payments:result.items,pagination:result.pagination});
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/logout") {
    const token=cookies(req).vanguardprime_session;if(token)db.prepare("DELETE FROM sessions WHERE token_hash=?").run(sha256(token));
  ...
  if (req.method === "POST" && url.pathname === "/api/wallet/deposit-session") {
    ...
    for(const [id,depositSession] of depositSessions)if(depositSession.expiresAt<=Date.now())depositSessions.delete(id);
    depositSessions.set(sessionId,{userId:user.id,amount,method:asset.method,symbol:asset.symbol,expiresAt});
    return json(res,201,{sessionId,asset:{symbol:asset.symbol,name:asset.name,network:asset.network,address:asset.address},expiresAt:new Date(expiresAt).toISOString()});
  }
  if (req.method === "POST" && url.pathname === "/api/wallet/deposits") {
    const user=requireUser(req,res);if(!user)return;const input=await body(req),amount=cents(input.amount),method=String(input.method||"").trim(),reference=String(input.reference||"").trim(),sessionId=String(input.sessionId||"").trim();
    if(!Number.isSafeInteger(amount)||amount<100)return json(res,422,{error:"Deposit must be at least $1.00."});
  ...
  if (req.method === "GET" && url.pathname === "/api/stock-orders") {
    const user=requireUser(req,res);if(!user)return;
    const page=pageRequest(url),result=pageResult(db.prepare("SELECT public_id,symbol,shares_micros,execution_price_cents,principal_cents,fee_cents,annual_roi_bps,duration_days,status,created_at FROM stock_orders WHERE user_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?").all(user.id,page.queryLimit,page.offset),page),orders=result.items.map(order=>({...order,...projectedValue(order.principal_cents,order.annual_roi_bps,order.created_at),elapsedDays:Math.max(0,Math.floor((Date.now()-new Date(order.created_at).getTime())/86400000))}));
    return json(res,200,{orders,pagination:result.pagination});
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/investment-orders") {
    const user=requireUser(req,res);if(!user)return;const page=pageRequest(url),result=pageResult(db.prepare("SELECT o.public_id,o.amount_cents,o.fee_cents,o.units_micros,o.duration_days,o.term_days,o.status,o.created_at,p.name AS plan_name,p.projected_return_bps,p.management_fee_bps,p.risk_level FROM investment_orders o JOIN investment_plans p ON p.id=o.plan_id WHERE o.user_id=? ORDER BY o.created_at DESC LIMIT ? OFFSET ?").all(user.id,page.queryLimit,page.offset),page),orders=result.items.map(order=>({...order,...investmentValue(order)}));return json(res,200,{orders,pagination:result.pagination});
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/dashboard") {
    ...
    const investmentOrders=db.prepare("SELECT o.public_id,o.amount_cents,o.fee_cents,o.duration_days,o.term_days,o.status,o.created_at,p.name AS plan_name,p.projected_return_bps FROM investment_orders o JOIN investment_plans p ON p.id=o.plan_id WHERE o.user_id=? ORDER BY o.created_at DESC LIMIT 25").all(user.id);
    const stocks=db.prepare("SELECT public_id,symbol,shares_micros,principal_cents,fee_cents,annual_roi_bps,duration_days,status,created_at FROM stock_orders WHERE user_id=? ORDER BY created_at DESC LIMIT 25").all(user.id);
