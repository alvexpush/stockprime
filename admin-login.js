(function(){
  ...
  (async()=>{if(!localStorage.getItem("vanguardprimeAdminSession"))return;try{const response=await fetch("/api/admin/payments",{credentials:"same-origin"});if(response.ok){location.replace("admin.html");return}}catch{}localStorage.removeItem("vanguardprimeAdminSession")})();
  ...
  form.onsubmit=async event=>{
    ...
    try{
      ...
      localStorage.setItem("vanguardprimeAdminSession",JSON.stringify({...data.admin,signedInAt:new Date().toISOString()}));location.replace("admin.html");
