...
const port=3224,base=`http://127.0.0.1:${port}`,databasePath=path.join(os.tmpdir(),`stockprime-deposit-session-${process.pid}.sqlite`),email=`deposit.session.${Date.now()}@example.com`;
...
(async()=>{try{
  ...
  const timestamp=new Date().toISOString(),user=db.prepare("INSERT INTO users (public_id,name,email,password_hash,country,currency,email_verified_at,created_at,updated_at) VALUES (?,?,?,?,?,'USD',?,?,?)").run(`USR-${crypto.randomUUID()}`,"Deposit Session",email,"test-hash","Nigeria",timestamp,timestamp,timestamp),token=crypto.randomBytes(32).toString("base64url"),tokenHash=crypto.createHash("sha256").update(token).digest("hex");db.prepare("INSERT INTO wallets (user_id,currency,created_at,updated_at) VALUES (?,'USD',?,?)").run(user.lastInsertRowid,timestamp,timestamp);db.prepare("INSERT INTO sessions (user_id,token_hash,expires_at,created_at) VALUES (?,?,?,?)").run(user.lastInsertRowid,tokenHash,new Date(Date.now()+3600000).toISOString(),timestamp);const cookie=`stockprime_session=${token}`;
