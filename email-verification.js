(function(){
  ...
  let state;try{state=JSON.parse(sessionStorage.getItem("vanguardprimeVerification")||"null")}catch{state=null}
  ...
  form.addEventListener("submit",async event=>{
    ...
    try{
      ...
      localStorage.setItem("vanguardprimeSession",JSON.stringify({email:data.user.email,name:data.user.name,signedInAt:new Date().toISOString(),serverAuthenticated:true}));
      const existing=JSON.parse(localStorage.getItem("vanguardprimeProfile")||"null")||{};localStorage.setItem("vanguardprimeProfile",JSON.stringify({...existing,name:data.user.name,email:data.user.email,phone:data.user.phone,country:data.user.country,currency:data.user.currency}));
      const destination=pagePurpose==="login"&&/^[a-z0-9._?=&-]+$/i.test(state.next||"")?state.next:"dashboard.html";sessionStorage.removeItem("vanguardprimeVerification");show(data.message,"success");button.textContent="Confirmed";setTimeout(()=>location.replace(destination),850);
